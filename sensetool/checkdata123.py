...
def check_image_correct(dataset, image_root):
    cnt_fail = 0
    cnt_success = 0
    ret = True
    for data in dataset:
...
def initialize_fn():
    client = AossClient()
    context[threading.current_thread()] = client
...
def checkdata(meta_dataset):
    with concurrent.futures.ProcessPoolExecutor(max_workers=16, initializer=initialize_fn) as executor:
...
# 数据脚本验证
def checkfiles(filepath, client=None):
    '''
